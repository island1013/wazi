...
class item_cateModel extends Model
{
    ...
    public function cate_cache() {
        $cate_list = array();
        $cate_data = $this->field('id,pid,name,type')->where('status=1')->order('ordid')->select();
    ...
    public function cate_data_cache() {
        $cate_data = array();
        $result = $this->field('id,pid,spid,name,type,seo_title,seo_keys,seo_desc')->where('status=1')->order('ordid')->select();
