    ...
    $.pinphp.item = {
        ...
        like: function(){
            ...
            $(s.like_btn).live('click', function(){
                ...
                var self = $(this),
                    ...
                    n = parseInt(nb.html());
                $.getJSON('?m=item&a=like', {id:id, aid:aid}, function(result){
                    if(result.status == 1){
                        //喜欢成功
                        nb.text(n+1).show();
        ...
        unlike: function(){
            ...
            $(s.unlike_btn).live('click', function(){
                ...
                var self = $(this),
                    id = self.attr('data-id');
                $.getJSON('?m=item&a=unlike', {id:id}, function(result){
                    if(result.status == 1){
        ...
        del: function(){
            ...
            $(s.del_btn).live('click', function(){
                ...
                $.getJSON('?m=item&a=delete', {id:id, album_id:album_id}, function(result){
                    if(result.status == 1){
                        self.parents(s.item_unit).slideUp(500, function(){
                            $(this).parent().masonry('remove', $(this)).masonry('reload');
                        });
        ...
        joinalbum: function(id){
            ...
            $(s.joinalbum_btn).live('click', function(){
                ...
                $.getJSON('?m=album&a=join', {id:id}, function(result){
                    if(result.status == 1){
        ...
        share: function(){
            ...
            $(s.share_btn).live('click', function(){
                ...
                $.getJSON('?m=item&a=share_item', function(result){
                    if(result.status == 1){
        ...
        share_form: function(form, aid){
            form.find('.J_si_btn').die('click').live('click', function(){
                ...
                $.getJSON('?m=item&a=fetch_item', {url:si_url, aid:aid}, function(result){
                    if(result.status == 1){
