    ...
    $.pinphp.topic = {
        ...
        piczoom: function(){
            ...
            $(s.topic_unit).find('.J_pic').live('click', function(){
                ...
                if(self.siblings('.J_src').html()){
                    ...
                    $.getJSON('?m=space&a=feed_src', {src_id:src_id, src_type:src_type}, function(result){
                        if(result.status == 1){
                            self.hide();
        ...
        delete: function(){
            ...
            $(s.topic_unit).find('.J_del').live('click', function(){
                ...
                $.getJSON('?m=topic&a=delete', {tid:tid}, function(result){
                    if(result.status == 1){
        ...
        forward: function(){
            ...
            $(s.topic_unit).find('.J_fw').live('click', function(){
                ...
                $.getJSON('?m=topic&a=forward', {tid:tid}, function(result){
                    if(result.status == 1){
        ...
        comment_list: function(unit){
            ...
            $.getJSON('?m=topic&a=comment', {tid:tid}, function(result){
                if(result.status == 1){
        ...
        comment_del: function(){
            ...
            $(s.cmt_unit).find('.J_cmt_del').live('click', function(){
                ...
                $.getJSON('?m=topic&a=comment_del', {cid:cid}, function(result){
                    if(result.status == 1){
                        cmt_i.remove();
                    }else{
                        $.pinphp.tip({content:result.msg, icon:'error'});
