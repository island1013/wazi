    ...
    $.pinphp.album = {
        ...
        form_init: function(form) {
            ...
            form.find('#J_upload_banner').uploader({
                action_url: '?m=album&a=album_upload_banner',
                input_id: 'J_banner',
                input_name: 'banner',
        ...
        create: function(){
            ...
            $.getJSON('?m=album&a=create_album', function(result){
                if(result.status == 1){
        ...
        edit: function(aid){
            ...
            $.getJSON('?m=album&a=edit_album', {aid:aid}, function(result){
                if(result.status == 1){
        ...
        del: function(aid){
            ...
            $.getJSON('?m=album&a=delete_album', {aid:aid}, function(result){
                if(result.status == 1){
                    $.pinphp.tip({content:result.msg});
                    window.location.reload();
                }else{
        ...
        follow: function(){
            $('.J_follow_album').live('click', function(){
                ...
                $.getJSON('?m=album&a=follow', {aid:aid}, function(result){
                    if(result.status == 1){
                        //更改按钮&添加数量
        ...
        unfollow: function(){
            $('.J_unfollow_album').live('click', function(){
                ...
                var self = $(this),
                    aid = self.attr('data-aid'),
                    afn = parseInt($('#J_afn_'+aid).text());
                $.getJSON('?m=album&a=unfollow', {aid:aid}, function(result){
                    if(result.status == 1){
                        //更改按钮&添加数量
